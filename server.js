...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        if (!parts.length) parts.push({ text: 'Hello' });
        ...
        if (!text) return res.status(502).json({ error: 'Gemini returned no text response.' });
        res.json({ text });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Unable to contact Gemini. Check the server logs.' });
    }
});

app.get('*', (req, res) => res.sendFile(path.join(__dirname, 'public', 'index.html')));
app.listen(PORT, () => console.log(`Chatbot server running at http://localhost:${PORT}`));
